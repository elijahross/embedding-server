use crate::ai::tokenization::EncodingInput;
...
impl ToSchema for PredictInput {
    fn schemas(schemas: &mut Vec<(String, utoipa::openapi::RefOr<utoipa::openapi::Schema>)>) {
        let _ = <Self as PartialSchema>::schema();
        // No need to push anything to schemas here unless you want to register custom schemas
    }
}

impl PartialSchema for PredictInput {
    ...
    fn schema() -> RefOr<Schema> {
        utoipa::openapi::ObjectBuilder::new()
            .property(
                ...
                utoipa::openapi::ObjectBuilder::new()
                    .schema_type(utoipa::openapi::Type::String)
                    .description(Some("A description of the error that occurred.")),
            )
            .property(
                "error_type",
                utoipa::openapi::ObjectBuilder::new()
                    .schema_type(utoipa::openapi::Type::String)
                    .description(Some("The type of error that occurred.")),
            )
            .required("error")
            .required("error_type")
            .description(Some("An error response."))
            .examples(Some(
