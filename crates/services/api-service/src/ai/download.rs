...
async fn download_file(api: &ApiRepo, file_path: &str) -> Result<PathBuf> {
    ...
    api.get(file_path)
        .await
        .map_err(|err| Error::Custom(format!("Failed to download `{}`: {:?}", file_path, err)))
}

async fn download_st_config_legacy(api: &ApiRepo) -> Result<PathBuf> {
...
#[instrument(skip_all)]
pub async fn download_artifacts(api: &ApiRepo, pool_config: bool) -> Result<PathBuf> {
