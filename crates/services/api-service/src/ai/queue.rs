use crate::ai::infer::InferResult;
use crate::ai::tokenization::ValidEncoding;
use crate::error::Result;
use lib_embedding::core::Batch;
use std::cmp::max;
...
pub struct Metadata {
    ...
    pub(crate) response_tx: oneshot::Sender<Result<InferResult>>,
...
impl Queue {
    ...
    pub fn append(&self, entry: Entry) -> Result<()> {
            ...
            .try_send(QueueCommand::Append(Box::new(entry), Span::current()))
        {
            ...
            Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                // Queue full — backpressure or 429
                error!("Queue is full");
                Err(crate::error::Error::Custom("Too many requests".into()))
            }
            Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
                error!("Queue background task terminated unexpectedly");
                // Receiver dropped — real bug
                Err(crate::error::Error::Custom(
                    "Queue background task terminated unexpectedly".into(),
                ))
            }
        }
    }
    ...
    pub fn len(&self) -> usize {
        // This is a best-effort approximation since we can't get the exact length of the queue
        // without additional state management.
        // Here, we return the number of available permits in the channel buffer.
        self.queue_sender.capacity()
    }

    pub async fn drain_pending(&self) -> Vec<Entry> {
        let (response_sender, response_receiver) = oneshot::channel();
        let _ = self
            .queue_sender
            .send(QueueCommand::DrainPending {
                response_sender,
                span: Span::current(),
            })
            .await;
    ...
    pub async fn next_batch(&self) -> Result<Option<NextBatch>> {
        let (response_sender, response_receiver) = oneshot::channel();
        match self.queue_sender.try_send(QueueCommand::NextBatch {
            response_sender,
            span: Span::current(),
        }) {
            Ok(_) => (),
            Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                return Err(crate::error::Error::Custom("Too many requests".into()));
            }
            Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
        ...
        response_receiver.await.map_err(|_| {
            crate::error::Error::Custom(
                "Queue background task dropped the sender without sending a new batch".into(),
            )
        })
    }
}
...
fn queue_blocking_task(
    ...
    while let Some(cmd) = queue_receiver.blocking_recv() {
        match cmd {
            ...
            QueueCommand::DrainPending {
                response_sender,
                span,
            } => {
                let _span = span.entered();
                let drained: Vec<Entry> = entries.drain(..).collect();
                let _ = response_sender.send(drained);
                let gauge = metrics::gauge!("te_queue_size");
...
enum QueueCommand {
    ...
    DrainPending {
        response_sender: oneshot::Sender<Vec<Entry>>,
        span: Span,
    },
}
