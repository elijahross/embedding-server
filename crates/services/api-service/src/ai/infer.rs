use crate::ai::queue::{Entry, Metadata, NextBatch, Queue};
use crate::ai::tokenization::{EncodingInput, RawEncoding, Tokenization};
use crate::error::{Error, Result};
use lib_embedding::InferenceBackend as Backend;
use lib_embedding::core::{Embedding, ModelType};
use std::sync::Arc;
...
impl Infer {
    ...
    ) -> Result<(Option<String>, RawEncoding)> {
        self.tokenization
    ...
    pub fn try_acquire_permit(&self) -> Result<OwnedSemaphorePermit> {
    ...
    ) -> Result<AllEmbeddingsInferResponse> {
    ...
    async fn embed<I: Into<EncodingInput> + std::fmt::Debug>(
        ...
        if let Err(_) = self.queue.append(Entry {
            metadata: Metadata {
                response_tx,
            ...
            metrics::counter!("te_request_failure", "err" => "queue_full").increment(1);
            return Err(Error::Custom("Queue is full. Please retry.".to_string()));
        }
    ...
    pub async fn predict<I: Into<EncodingInput> + std::fmt::Debug>(
        ...
        raw_scores: bool,
        _permit: OwnedSemaphorePermit,
    ) -> Result<ClassificationInferResponse> {
        if !self.is_classifier() {
            let counter = metrics::counter!("te_request_failure", "err" => "model_type");
            counter.increment(1);
    ...
    pub fn is_splade(&self) -> bool {
        matches!(
            ...
            ModelType::Embedding(lib_embedding::Pool::Splade)
        )
    }
...
async fn batching_task(queue: Queue, notify: Arc<Notify>, embed_sender: mpsc::Sender<NextBatch>) {
    loop {
        ...
        {
            // Try to reserve capacity in embed_sender
            let mut permit = match embed_sender.reserve().await {
                Ok(permit) => permit,
                Err(_) => {
                    tracing::error!("embed receiver was dropped. Exiting batching task.");
                    return;
                }
            };
            ...
            loop {
                match queue.next_batch().await {
                    Ok(Some(next_batch)) => {
                        ...
                        permit = match embed_sender.reserve().await {
                            Ok(p) => p,
                            Err(_) => {
                                tracing::error!(
                                    "embed receiver was dropped. Exiting batching task."
                                );
                                return;
                            }
                        };
                    ...
                    Err(_) => {
                        // Queue is full (or some other error)
                        tracing::error!("Queue is full, returning error to waiting requests");

                        // Drain waiting items and return an error immediately
                        let drained = queue.drain_pending().await;
                        for entry in drained {
                            let _ = entry
                                .metadata
                                .response_tx
                                .send(Err(Error::Custom("Queue is full".to_string())));
                        }

                        // Break so we don’t keep looping endlessly
                        break;
                    }
                }
            }
                ...
                std::thread::spawn(move || match results {
                    ...
                    Err(err) => {
                        batch.0.into_iter().for_each(|m| {
                            let _ = m.response_tx.send(Err(Error::Custom(err.to_string())));
                        });
                    }
                });
            }
