// coding=utf-8
// Following code is adapted from the Hugging Face `text-embeddings-inference` repository.
// Original repo can be found at https://github.com/huggingface/text-embeddings-inference/
...
use crate::core::{InferenceBackend as CoreBackend, Predictions};
...
pub use crate::core::{Batch, Embedding, Embeddings, ModelType, Pool};
pub use crate::dtype::DType;
pub use crate::error::{Error as BackendError, Result};
...
impl InferenceBackend {
    ...
    pub async fn warmup_hpu(
        ...
        if max_warmup_length > max_input_length {
            return Err(BackendError::Start(format!(
                "max_warmup_length ({max_warmup_length}) exceeds model's max_input_length ({max_input_length}), you can modify this value adding `-e MAX_WARMUP_SEQUENCE_LENGTH=<new_warmup_length>` to your Docker run command"
            )));
        }
        if seq_bucket_size > max_warmup_length {
            return Err(BackendError::Start(format!(
                "PAD_SEQUENCE_TO_MULTIPLE_OF ({seq_bucket_size}) exceeds model's max warmup length ({max_warmup_length}), you can modify these values adding `-e PAD_SEQUENCE_TO_MULTIPLE_OF=<new_value>` or `-e MAX_WARMUP_SEQUENCE_LENGTH=<new_value> to your Docker run command`"
            )));
        }
    ...
    #[instrument(skip_all)]
    pub async fn embed(&self, batch: Batch) -> Result<(Embeddings, Duration)> {
        let (sender, receiver) = oneshot::channel();
    ...
    #[instrument(skip_all)]
    pub async fn predict(&self, batch: Batch) -> Result<(Predictions, Duration)> {
        let (sender, receiver) = oneshot::channel();
...
async fn init_backend(
        ...
        #[cfg(feature = "ort")]
        {
            if let Some(api_repo) = api_repo.as_ref() {
                ...
                match model_files.is_empty() {
                    true => {
                        tracing::error!(
                            "Model ONNX files not found in the repository. You can easily create ONNX files using the following scripts: https://gist.github.com/tomaarsen/4b00b0e3be8884efa64cfab9230b161f, or use this Space: https://huggingface.co/spaces/sentence-transformers/backend-export"
                        )
                    }
                    false => {
            ...
            let backend = ort::OrtBackend::new(&model_path, dtype.to_string(), model_type.clone());
            match backend {
    ...
    if cfg!(feature = "candle") {
        #[cfg(feature = "candle")]
        {
            ...
            let backend = candle::CandleBackend::new(
                &model_path,
                dtype.to_string(),
                model_type.clone(),
                dense_paths,
...
enum BackendCommand {
    Health(Span, oneshot::Sender<Result<()>>),
    Embed(Batch, Span, oneshot::Sender<Result<(Embeddings, Duration)>>),
    Predict(
        Batch,
        Span,
        #[allow(clippy::type_complexity)] oneshot::Sender<Result<(Predictions, Duration)>>,
    ),
}

async fn download_safetensors(api: &ApiRepo) -> Result<Vec<PathBuf>> {
    // Single file
...
#[cfg(feature = "ort")]
async fn download_onnx(api: &ApiRepo) -> Result<Vec<PathBuf>> {
...
#[cfg(feature = "candle")]
async fn download_file(api: &ApiRepo, file_path: &str) -> Result<PathBuf> {
    tracing::info!("Downloading `{}`", file_path);
    api.get(file_path)
        .await
        .map_err(|err| BackendError::Custom(err.to_string()))
}

#[cfg(feature = "candle")]
async fn parse_dense_paths_from_modules(modules_path: &PathBuf) -> Result<Vec<String>> {
    let content = std::fs::read_to_string(modules_path)?;
    let modules: Vec<ModuleConfig> =
        serde_json::from_str(&content).map_err(|err| BackendError::Custom(err.to_string()))?;
    ...
    match download_file(api, "modules.json").await {
        Ok(modules_path) => {
            ...
            match parse_dense_paths_from_modules(&modules_path).await {
                    ...
                    match module_paths.len() {
                        ...
                        1 => {
                            let path_to_use = if let Some(ref user_path) = dense_path {
                                if user_path != &module_paths[0] {
                                    tracing::info!(
                                        "`{}` found in `modules.json`, but using provided `--dense-path={user_path}` instead",
                                        module_paths[0]
                                    );
                                }
                                user_path.clone()
                        ...
                        _ => {
                            if dense_path.is_some() {
                                tracing::warn!(
                                    "A value for `--dense-path` was provided, but since there's more than one subsequent Dense module, then the provided value will be ignored."
                                );
                            }
                ...
                Err(err) => {
                    tracing::warn!(
                        "`modules.json` could be downloaded but parsing the modules failed: {err}; so no Dense modules will be downloaded."
                    );
                    Ok(vec![])
                }
            }
...
#[cfg(feature = "candle")]
async fn download_dense_module(api: &ApiRepo, dense_path: &str) -> Result<PathBuf> {
