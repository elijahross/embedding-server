use crate::core::{Batch, Embedding, Embeddings, InferenceBackend, ModelType, Pool, Predictions};
...
impl OrtBackend {
    pub fn new(model_path: &Path, dtype: String, model_type: ModelType) -> Result<Self> {
        if dtype != "float32" {
        ...
        let session = Session::builder()
            .map_err(|e| {
                BackendError::Start(format!("Failed to create ONNX session builder: {e}"))
            })?
            .with_intra_threads(num_cpus::get())
            .map_err(|e| BackendError::Start(format!("Failed to set intra threads: {e}")))?
            .with_optimization_level(GraphOptimizationLevel::Level3)
            .map_err(|e| BackendError::Start(format!("Failed to set optimization level: {e}")))?
            .commit_from_file(onnx_path)
            .map_err(|e| {
                BackendError::Start(format!("Failed to commit ONNX session from file: {e}"))
            })?;
...
impl OrtBackend {
    fn prepare_inputs(
        ...
        let input_ids = ndarray::Array2::from_shape_vec((batch_size, max_length), input_ids)?;
        let attention_mask =
            ndarray::Array2::from_shape_vec((batch_size, max_length), attention_mask)?;
    ...
    fn prepare_ort_inputs(
        ...
        if let Some(token_type_ids) = token_type_ids {
            let token_type_ids = ort::value::Tensor::from_array(token_type_ids)?;
            inputs.push((
        ...
        if let Some(position_ids) = position_ids {
            let position_ids = ort::value::Tensor::from_array(position_ids)?;
        ...
        if let Some(past_key_values) = past_key_values {
            for (layer_idx, (key, value)) in past_key_values.into_iter().enumerate() {
                let key = ort::value::Tensor::from_array(key)?;
                let value = ort::value::Tensor::from_array(value)?;

                inputs.push((
                    format!("past_key_values.{layer_idx}.key").into(),
                    key.into(),
                ));
                inputs.push((
                    format!("past_key_values.{layer_idx}.value").into(),
                    value.into(),
                ));
            }
        }
...
impl InferenceBackend for OrtBackend {
    ...
    fn embed(&self, batch: Batch) -> Result<Embeddings> {
        ...
        if outputs.contains_key("sentence_embedding") {
            ...
            if has_raw_requests {
                ...
                #[allow(deprecated)]
                let outputs = outputs.into_shape((s[0] * s[1], s[2]))?;
    ...
    fn predict(&self, batch: Batch) -> Result<Predictions> {
        ...
        let outputs = outputs["logits"].try_extract_array::<f32>()?.to_owned();
