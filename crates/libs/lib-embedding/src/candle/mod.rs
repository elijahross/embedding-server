...
use crate::core::{Batch, Embedding, Embeddings, InferenceBackend, ModelType, Predictions};
use crate::error::{Error as BackendError, Result};
...
use crate::candle::models::{
    BertConfig, BertModel, Dense, DenseConfig, DenseLayer, DistilBertConfig, DistilBertModel,
    GTEConfig, GTEModel, Gemma3Config, Gemma3Model, MistralConfig, Model, NomicBertModel,
    NomicConfig, Qwen3Config, Qwen3Model,
};
#[cfg(feature = "cuda")]
...
#[cfg(feature = "cuda")]
use crate::models::{
    FlashBertModel, FlashDistilBertModel, FlashGTEModel, FlashMistralModel, FlashNomicBertModel,
    FlashQwen3Model,
};
...
impl CandleBackend {
    pub fn new(
        ...
        let config: Config =
            serde_json::from_str(&config).map_err(|err| BackendError::Start(format!("{err:?}")))?;
        ...
        let model: Result<Box<dyn Model + Send>> = match (config, &device) {
            #[cfg(not(feature = "cuda"))]
            ...
            (
                ...
                Ok(Box::new(BertModel::load_roberta(vb, &config, model_type)?))
            }
            (Config::Bert(config), Device::Cpu | Device::Metal(_)) => {
                let bert_config = match &config {
                    BertConfigWrapper::JinaBert(inner) => inner,
                    BertConfigWrapper::JinaCodeBert(inner) => inner,
                    BertConfigWrapper::Bert(inner) => inner,
                };
                tracing::info!("Starting Bert model on {:?}", device);
                Ok(Box::new(BertModel::load(vb, bert_config, model_type)?))
            }
            (Config::DistilBert(config), Device::Cpu | Device::Metal(_)) => {
                tracing::info!("Starting DistilBert model on {:?}", device);
                Ok(Box::new(DistilBertModel::load(vb, &config, model_type)?))
            }
            (Config::Gemma3(config), Device::Cpu | Device::Metal(_)) => {
                if dtype != DType::F32 {
                    ...
                    Ok(Box::new(Gemma3Model::load(vb, &config, model_type)?))
                }
            }
            (Config::Gte(config), Device::Cpu | Device::Metal(_)) => {
                tracing::info!("Starting GTE model on {:?}", device);
                Ok(Box::new(GTEModel::load(vb, &config, model_type)?))
            }
            ...
            (Config::NomicBert(config), Device::Cpu | Device::Metal(_)) => {
                ...
                Ok(Box::new(NomicBertModel::load(vb, &config, model_type)?))
            }
            (Config::Qwen3(config), Device::Cpu | Device::Metal(_)) => {
                tracing::info!("Starting Qwen3 model on {:?}", device);
                Ok(Box::new(Qwen3Model::load(vb, &config, model_type)?))
            }
            #[cfg(feature = "cuda")]
            ...
            (
                ...
                {
                    tracing::info!("Starting FlashBert model on {:?}", device);
                    Ok(Box::new(FlashBertModel::load_roberta(
                        vb, &config, model_type,
                    )?))
                } else {
                    tracing::info!("Starting Bert model on {:?}", device);
                    Ok(Box::new(BertModel::load_roberta(vb, &config, model_type)?))
            ...
            (Config::DistilBert(config), Device::Cuda(_)) => {
                        ...
                        == "true"
                {
                    ...
                    Ok(Box::new(FlashDistilBertModel::load(
                        vb, &config, model_type,
                    )?))
                } else {
            ...
            (Config::Gte(config), Device::Cuda(_)) => {
                        ...
                        != "true"
                {
                    ...
                    Ok(Box::new(FlashGTEModel::load(vb, &config, model_type)?))
                }
            }
            #[cfg(feature = "cuda")]
            (Config::Mistral(config), Device::Cuda(_)) => {
                ...
                Ok(Box::new(FlashMistralModel::load(vb, &config, model_type)?))
            }
            #[cfg(feature = "cuda")]
            (Config::NomicBert(config), Device::Cuda(_)) => {
                        ...
                        == "true"
                {
                    ...
                    Ok(Box::new(FlashNomicBertModel::load(
                        vb, &config, model_type,
                    )?))
                } else {
                    tracing::info!("Starting NomicBert model on {:?}", device);
            ...
            (Config::Qwen3(config), Device::Cuda(_)) => {
                        ...
                        != "true"
                {
                    ...
                    Ok(Box::new(FlashQwen3Model::load(vb, &config, model_type)?))
                }
            }
        };
                ...
                for dense_path in dense_paths.iter() {
                    ...
                    if dense_safetensors.exists() || dense_pytorch.exists() {
                        ...
                        let dense_vb = if dense_safetensors.exists() {
                            ...
                            VarBuilder::from_pth(&dense_pytorch, dtype, &device)?
                        };

                        let dense_layer = Box::new(Dense::load(dense_vb, &dense_config)?)
...
impl InferenceBackend for CandleBackend {
    ...
    fn embed(&self, batch: Batch) -> Result<Embeddings> {
        ...
        // Run forward
        let (pooled_embeddings, raw_embeddings) = self.model.embed(batch)?;
        ...
        let pooled_embeddings = match pooled_embeddings {
            ...
            Some(mut pooled_embeddings) => {
                for dense in &self.dense_layers {
                    pooled_embeddings = dense.forward(&pooled_embeddings)?;
                }
        ...
        let pooled_embeddings = match pooled_embeddings {
            None => vec![],
            Some(pooled_embeddings) => pooled_embeddings.to_dtype(DType::F32)?.to_vec2()?,
        };

        // This transfer is expensive...
        let raw_embeddings = match raw_embeddings {
            None => vec![],
            Some(raw_embeddings) => raw_embeddings.to_dtype(DType::F32)?.to_vec2()?,
        };
    ...
    fn predict(&self, batch: Batch) -> Result<Predictions> {
        ...
        let results = self.model.predict(batch)?;

        let results = results.to_dtype(DType::F32)?.to_vec2()?;
