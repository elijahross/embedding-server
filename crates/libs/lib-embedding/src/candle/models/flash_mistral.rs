...
use crate::candle::models::{MistralConfig, Model};
...
impl FlashMistralModel {
    pub fn load(vb: VarBuilder, config: &MistralConfig, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashMistral requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashMistral requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                candle_core::bail!("`classifier` model type is not supported for Mistral")
            }
