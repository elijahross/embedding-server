use crate::candle::layers::{
    HiddenAct, Linear, apply_rotary, get_cos_sin, get_cublas_lt_wrapper, get_inv_freqs,
};
        ...
        let pooled_embeddings = if has_pooling_requests {
            match self.pool {
                Pool::Cls | Pool::LastToken | Pool::Splade => {
                    unreachable!(
                        "Only Mean Pooling is supported for Gemma3, neither CLS, nor Last-Token, nor SPLADE"
                    );
                }
                Pool::Mean => {
                    if batch_size > 1 {
