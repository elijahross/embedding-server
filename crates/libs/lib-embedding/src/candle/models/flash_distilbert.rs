...
use crate::candle::models::distilbert::{
...
impl FlashDistilBertModel {
    pub fn load(vb: VarBuilder, config: &DistilBertConfig, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashDistilBert requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashDistilBert requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                candle_core::bail!("`classifier` model type is not supported for DistilBert")
            }
