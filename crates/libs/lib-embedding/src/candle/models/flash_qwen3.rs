...
use crate::candle::layers::{HiddenAct, Linear, RMSNorm, get_cos_sin, get_inv_freqs};
use crate::candle::models::{Model, Qwen3Config};
...
impl Qwen3Attention {
    pub fn load(vb: VarBuilder, config: &Qwen3Config) -> Result<Self> {
        if config.use_sliding_window {
            candle_core::bail!("Sliding window is not supported for Qwen3");
        }
...
impl FlashQwen3Model {
    pub fn load(vb: VarBuilder, config: &Qwen3Config, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashQwen3 requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashQwen3 requires DType::F16")
        }

        let pool = match model_type {
            ModelType::Classifier => {
                candle_core::bail!("`classifier` model type is not supported for Qwen3")
            }
