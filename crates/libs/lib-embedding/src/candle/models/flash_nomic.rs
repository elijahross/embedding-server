...
use crate::candle::models::nomic::{NomicBertEmbeddings, NomicMLP};
use crate::candle::models::{Model, NomicConfig};
...
impl FlashNomicBertModel {
    pub fn load(vb: VarBuilder, config: &NomicConfig, model_type: ModelType) -> Result<Self> {
        ...
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashNomicBertModel requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashNomicBertModel requires DType::F16")
        }
