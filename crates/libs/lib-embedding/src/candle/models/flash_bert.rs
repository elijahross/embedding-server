...
use crate::candle::layers::{LayerNorm, Linear};
use crate::candle::models::Model;
use crate::candle::models::bert::{
...
impl BertAttention {
    ...
    pub fn forward(
        ...
        let attention = attention.flatten_from(candle_core::D::Minus2)?;
...
impl FlashBertModel {
    pub fn load(vb: VarBuilder, config: &BertConfig, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashBert requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashBert requires DType::F16")
        }

        // Check position embedding type
        if config.position_embedding_type != PositionEmbeddingType::Absolute {
            candle_core::bail!("FlashBert only supports absolute position embeddings")
        }
