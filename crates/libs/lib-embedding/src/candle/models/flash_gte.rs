use crate::candle::flash_attn::flash_attn_varlen;
use crate::candle::layers::{LayerNorm, Linear, get_cos_sin, get_inv_freqs};
use crate::candle::models::gte::{GTEClassificationHead, GTEConfig, GTEMLP};
use crate::candle::models::{Model, PositionEmbeddingType};

use crate::core::{Batch, ModelType, Pool};
...
impl FlashGTEModel {
    pub fn load(vb: VarBuilder, config: &GTEConfig, model_type: ModelType) -> Result<Self> {
        match vb.device() {
            Device::Cuda(_) => {}
            _ => candle_core::bail!("FlashGTE requires Cuda"),
        }

        if vb.dtype() != DType::F16 {
            candle_core::bail!("FlashGTE requires DType::F16")
        }

        if config.logn_attention_clip1 {
            candle_core::bail!("`logn_attention_clip1` is not supported");
        }
        if config.logn_attention_scale {
            candle_core::bail!("`logn_attention_scale` is not supported");
        }

        if config.position_embedding_type != PositionEmbeddingType::Rope {
            candle_core::bail!("Only `PositionEmbeddingType::Rope` is supported");
        }
